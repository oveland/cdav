<div class="portlet-title">
    ...
    <span class="caption m-l-40 phase-status-container">
        ...
        <i class="icon-ghost font-yellow-lemon faa-flash tooltips m-l-10 pull-right"
           data-placement="bottom" data-original-title="@lang('In abandonment state')">
        </i>
    </span>

    <div class="actions">
        <button class="btn bnt-sm btn-icon-only yellow-lemon btn-circle ajax-btn-auto-phase-2 tooltips hide" style="text-transform: capitalize" data-phase="2"
                data-original-title="@lang('Set to Abandonment state')" data-action="{{ route('inventory-ajax', 'setAbandonmentstate') }}">
            <i class="icon-ghost faa-flash tooltips m-l-10 pull-right"></i>
        </button>
        <div class="btn-group" data-original-title="@lang('Download report')" data-placement="left">
            <button class="btn btn-circle green-seagreen dropdown-toggle btn-download" data-toggle="dropdown" aria-expanded="true">
                <i class="fa fa-download"></i>
                <i class="fa fa-angle-down"></i>
            </button>
            <ul class="dropdown-menu">
                <li>
                    <a href="{{ route('reports-personal-notification-phase-2') }}" class="tooltips" data-original-title="@lang('Report of') @lang('Personal notifications')">
                        <i class="icon-envelope-open faa-bounce font-blue-madison" aria-hidden="true"></i> | @lang('By Personal')
                    </a>
                </li>
                <li>
                    <a href="{{ route('reports-notice-notification-phase-2') }}" class="tooltips" data-original-title="@lang('Report of') @lang('Notice notifications')">
                        <i class="fa fa-bullhorn faa-bounce font-yellow-lemon" aria-hidden="true"></i> | @lang('By Notice')
                    </a>
                </li>
                <li>
                    <a href="{{ route('reports-phase-2') }}" class="tooltips" data-original-title="@lang('Download report') @lang('In abandonment')">
                        <i class="icon-ghost font-yellow-lemon"></i> | @lang('In abandonment')
                    </a>
                </li>
            </ul>
        </div>

        <button class="btn btn-circle btn-icon-only btn-default tooltips" data-container="body" onclick="Inventory.loadPhaseContainer($(this).parents('.phase-container'));" data-original-title="@lang('Update')">
            ...
            <tr class="odd gradeX">
                <td class="text-center">
                    @if($inventoryProcess->started)
                        <i class="icon-ghost font-yellow-crusta faa-flash animated tooltips"
                           data-original-title="@lang('In abandonment state')">
                        </i>
                    @else
